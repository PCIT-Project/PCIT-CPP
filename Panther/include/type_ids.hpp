...
namespace pcit::panther{
	...
	struct TypeInfoID : public core::UniqueID<uint32_t, struct TypeInfoID> {
		...
		[[nodiscard]] auto hash() const -> size_t { return std::hash<uint32_t>{}(this->get()); }


		// This is not the most elegant way of doing this, but I don't want to overcomplicate things for one place
		[[nodiscard]] static auto createTemplateDeclInstantiation() -> TypeInfoID {
			return TypeInfoID(std::numeric_limits<uint32_t>::max() - 1);
		}
		[[nodiscard]] auto isTemplateDeclInstantiation() const -> bool {
...
namespace pcit::panther{
	...
	class TypeInfoVoidableID{
		public:
			...
			[[nodiscard]] static auto Void() -> TypeInfoVoidableID { return TypeInfoVoidableID(); };

			[[nodiscard]] auto operator==(TypeInfoVoidableID rhs) const -> bool {
				return this->id == rhs.id;
			};

			[[nodiscard]] auto operator==(TypeInfoID rhs) const -> bool {
				return this->id == rhs;
			};

			[[nodiscard]] auto asTypeID() const -> const TypeInfoID& {
				evo::debugAssert(this->isVoid() == false, "type is void");
				return this->id;
			};

			[[nodiscard]] auto asTypeID() -> TypeInfoID& {
				evo::debugAssert(this->isVoid() == false, "type is void");
				return this->id;
			};
			...
			[[nodiscard]] auto hash() const -> size_t { return std::hash<uint32_t>{}(this->id.get()); }
...
namespace pcit::panther::BaseType{
	...
	struct InterfacePtrMapID : public core::UniqueID<uint32_t, struct InterfacePtrMapID> {
		using core::UniqueID<uint32_t, InterfacePtrMapID>::UniqueID; 
	};
...
namespace pcit::core{
	...
	template<>
	struct OptionalInterface<panther::BaseType::InterfacePtrMapID>{
		static constexpr auto init(panther::BaseType::InterfacePtrMapID* id) -> void {
			std::construct_at(id, std::numeric_limits<uint32_t>::max());
		}

		static constexpr auto has_value(const panther::BaseType::InterfacePtrMapID& id) -> bool {
			return id.get() != std::numeric_limits<uint32_t>::max();
		}
	};
...
namespace std{
	...
	template<>
	struct hash<pcit::panther::BaseType::InterfacePtrMapID>{
		auto operator()(const pcit::panther::BaseType::InterfacePtrMapID& id) const noexcept -> size_t {
			return hash<uint32_t>{}(id.get());
		};
	};
	template<>
	class optional<pcit::panther::BaseType::InterfacePtrMapID>
		: public pcit::core::Optional<pcit::panther::BaseType::InterfacePtrMapID>{

		public:
			using pcit::core::Optional<pcit::panther::BaseType::InterfacePtrMapID>::Optional;
			using pcit::core::Optional<pcit::panther::BaseType::InterfacePtrMapID>::operator=;
	};
...
namespace pcit::panther{
	...
	struct EncapsulatingSymbolID{
		struct InterfaceImplInfo{
			...
			[[nodiscard]] auto operator==(const InterfaceImplInfo&) const -> bool = default;
		};

		template<class T>
		[[nodiscard]] auto is() const -> bool { return this->id.is<T>(); }

		template<class T>
		[[nodiscard]] auto as() const -> const T& { return this->id.as<T>(); }

		template<class T>
		[[nodiscard]] auto as() -> T& { return this->id.as<T>(); }
		...
		[[nodiscard]] auto operator==(const EncapsulatingSymbolID&) const -> bool = default;
