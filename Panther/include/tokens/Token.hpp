...
namespace pcit::panther{
	...
	class Token{
		...
		public:
			...
			[[nodiscard]] auto kind() const -> const Kind& { return this->_kind; }
			...
			[[nodiscard]] auto getInt() const -> uint64_t {
			...
			[[nodiscard]] auto getFloat() const -> evo::float64_t {
			...
			[[nodiscard]] static consteval auto lookupKind(std::string_view op_str) -> Kind {
				// length 4
			...
			[[nodiscard]] static auto printKind(Kind kind) -> std::string_view {
				switch(kind){
					...
					break; case Kind::KEYWORD_SWITCH:         return "switch";
					break; case Kind::KEYWORD_CASE:           return "case";
		...
		private:
			template<class T> [[nodiscard]] auto get_value() const -> const T&;
			template<class T> [[nodiscard]] auto get_value()       ->       T&;

			template<> [[nodiscard]] auto get_value<bool>() const -> const bool& { return *(bool*)&this->value; }
			template<> [[nodiscard]] auto get_value<bool>()       ->       bool& { return *(bool*)&this->value; }

			template<> [[nodiscard]] auto get_value<char>() const -> const char& { return *(char*)&this->value; }
			template<> [[nodiscard]] auto get_value<char>()       ->       char& { return *(char*)&this->value; }

			template<> [[nodiscard]] auto get_value<uint64_t>() const -> const uint64_t& {
				return *(uint64_t*)&this->value;
			}
			template<> [[nodiscard]] auto get_value<uint64_t>()       ->       uint64_t& {
				return *(uint64_t*)&this->value;
			}

			template<> [[nodiscard]] auto get_value<evo::float64_t>() const -> const evo::float64_t& {
				return *(evo::float64_t*)&this->value;
			}
			template<> [[nodiscard]] auto get_value<evo::float64_t>()       ->       evo::float64_t& {
				return *(evo::float64_t*)&this->value;
			}

			template<> [[nodiscard]] auto get_value<SmallStringView>() const -> const SmallStringView& {
				return *(SmallStringView*)&this->value;
			}
			template<> [[nodiscard]] auto get_value<SmallStringView>()       ->       SmallStringView& {
				return *(SmallStringView*)&this->value;
			}
...
namespace std{
	...
	struct hash<pcit::panther::Token::ID> {
	    [[nodiscard]] auto operator()(pcit::panther::Token::ID id) const -> size_t {
