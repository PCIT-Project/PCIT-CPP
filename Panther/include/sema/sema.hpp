...
namespace pcit::panther::sema{
	...
	struct BlockExpr{
		...
		[[nodiscard]] auto hasNamedOutputs() const -> bool { return this->outputs[0].ident.has_value(); }
	};
	...
	struct Func{
		...
		[[nodiscard]] auto isEquivalentOverload(const Func& rhs, const class panther::Context& context) const -> bool;

		[[nodiscard]] auto isMethod(const class panther::Context& context) const -> bool;
	...
	struct TemplatedFunc{
		...
		struct Instantiation{
			...
			[[nodiscard]] auto errored() const -> bool { return this->errored_reason.is<std::monostate>() == false; }
