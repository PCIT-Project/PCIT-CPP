...
namespace pcit::panther::sema{
	...
	auto ScopeLevel::addIdent(std::string_view ident, sema::FuncID id, const Context& context) -> AddIdentResult {
		...
		if(ident_find == this->ids.end()){
			...
			new_ident_id.as<FuncOverloadList>().funcs.emplace_back(
			...
			if(ident_find->second.is<FuncOverloadList>()){
				OverloadList& overload_list = ident_find->second.as<FuncOverloadList>().funcs;
	...
	auto ScopeLevel::addIdent(std::string_view ident, MethodOverloadFlag, sema::FuncID id, const Context& context)
	-> AddIdentResult {
		const auto lock = std::scoped_lock(this->idents_lock);
		...
		const std::unordered_map<std::string_view, IdentID>::iterator ident_find = this->ids.find(ident);
		if(ident_find == this->ids.end()){
			IdentID& new_ident_id = this->ids.emplace(ident, IdentID(MethodOverloadList())).first->second;
			new_ident_id.as<MethodOverloadList>().funcs.emplace_back(
				evo::Variant<sema::FuncID, sema::TemplatedFuncID>(id)
			);
			return &new_ident_id;

		}else{
			if(ident_find->second.is<MethodOverloadList>()){
				OverloadList& overload_list = ident_find->second.as<MethodOverloadList>().funcs;
	...
	auto ScopeLevel::addIdent(std::string_view ident, sema::TemplatedFuncID id) -> AddIdentResult {
		...
		if(ident_find == this->ids.end()){
			...
			if(ident_find->second.is<FuncOverloadList>()){
				ident_find->second.as<FuncOverloadList>().funcs.emplace_back(
					evo::Variant<sema::FuncID, sema::TemplatedFuncID>(id)
				);
				return &ident_find->second;
			}else{
	...
	auto ScopeLevel::addIdent(std::string_view ident, MethodOverloadFlag, sema::TemplatedFuncID id) -> AddIdentResult {
		const auto lock = std::scoped_lock(this->idents_lock);
		...
		if(ident_find == this->ids.end()){
			...
			if(ident_find->second.is<MethodOverloadList>()){
				ident_find->second.as<MethodOverloadList>().funcs.emplace_back(
					evo::Variant<sema::FuncID, sema::TemplatedFuncID>(id)
				);
				return &ident_find->second;
			}else{
	...
	auto ScopeLevel::disallowIdentForShadowing(std::string_view ident, const IdentID* id) -> bool {
		...
		if(find != this->ids.end()){
			// make sure you can't shadow a member var or method
			return find->second.is<MemberVar>() || find->second.is<MethodOverloadList>();
		}
