...
namespace pcit::panther{
	...
	class SymbolProcBuilder{
		public:
			...
			[[nodiscard]] auto build(const AST::Node& stmt) -> evo::Result<SymbolProc::ID>;

			// for struct
			[[nodiscard]] auto buildTemplateInstance(
			...
			[[nodiscard]] auto buildInterfaceImplDeducer(
		...
		private:
			[[nodiscard]] auto get_symbol_ident(const AST::Node& stmt) -> evo::Result<std::string_view>;

			[[nodiscard]] auto build_var_def(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_func_def(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_deleted_special_method(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_func_alias_def(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_alias_def(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_struct_def(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_union_def(const AST::Node& stmt) -> evo::Result<>;
			[[nodiscard]] auto build_enum_def(const AST::Node& stmt) -> evo::Result<>;
			...
			[[nodiscard]] auto analyze_type(const AST::Type& ast_type) -> evo::Result<SymbolProc::TypeID>;

			template<bool NEEDS_DEF>
			[[nodiscard]] auto analyze_type_term(const AST::Type& ast_type) -> evo::Result<SymbolProc::TermInfoID>;

			template<bool NEEDS_DEF>
			[[nodiscard]] auto analyze_type_base(const AST::Node& ast_type_base) -> evo::Result<SymbolProc::TermInfoID>;
			...
			template<bool IS_COMPTIME>
			[[nodiscard]] auto analyze_expr_block(const AST::Node& node) -> evo::Result<SymbolProc::TermInfoID>;

			template<bool IS_COMPTIME, bool ERRORS>
			[[nodiscard]] auto analyze_expr_func_call(const AST::Node& node) -> evo::Result<SymbolProc::TermInfoID>;

			template<bool IS_COMPTIME>
			[[nodiscard]] auto analyze_expr_indexer(const AST::Node& node) -> evo::Result<SymbolProc::TermInfoID>;
			...
			[[nodiscard]] auto analyze_expr_intrinsic(const AST::Node& node) -> evo::Result<SymbolProc::TermInfoID>;

			template<bool NEEDS_DEF>
			[[nodiscard]] auto analyze_expr_type_this(const AST::Node& node) -> evo::Result<SymbolProc::TermInfoID>;

			[[nodiscard]] auto analyze_expr_literal(const Token::ID& literal) -> evo::Result<SymbolProc::TermInfoID>;
			[[nodiscard]] auto analyze_expr_uninit(const Token::ID& uninit_token)
				-> evo::Result<SymbolProc::TermInfoID>;
			[[nodiscard]] auto analyze_expr_zeroinit(const Token::ID& zeroinit_token)
				-> evo::Result<SymbolProc::TermInfoID>;
			[[nodiscard]] auto analyze_expr_this(const Token::ID& node) -> evo::Result<SymbolProc::TermInfoID>;


			[[nodiscard]] auto analyze_attributes(const AST::AttributeBlock& attribute_block)
				-> evo::Result<evo::SmallVector<SymbolProc::Instruction::AttributeParams>>;


			[[nodiscard]] auto analyze_priority_and_builtin_attribute(const AST::AttributeBlock& attribute_block)
				-> evo::Result<>;

			[[nodiscard]] auto analyze_template_param_pack(const AST::TemplatePack& template_pack)
				-> evo::Result<evo::SmallVector<SymbolProc::Instruction::TemplateParamInfo>>;


			[[nodiscard]] auto is_deducer(const AST::Node& node) const -> bool;
			[[nodiscard]] auto is_named_deducer(const AST::Node& node) const -> bool;

			[[nodiscard]] auto extract_deducer_names(const AST::Node& node) const 
			...
			[[nodiscard]] auto is_child_symbol() const -> bool { return this->symbol_proc_infos.size() > 1; }

			[[nodiscard]] auto get_parent_symbol() -> SymbolProcInfo& {
				evo::debugAssert(this->is_child_symbol(), "Not in child symbol");
				return this->symbol_proc_infos[this->symbol_proc_infos.size() - 2];
			}

			[[nodiscard]] auto get_current_symbol() -> SymbolProcInfo& {
	...
	[[nodiscard]] inline auto build_symbol_procs(Context& context, Source::ID source_id) -> evo::Result<> {
