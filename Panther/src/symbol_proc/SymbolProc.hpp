...
#include "../sema/TermInfo.hpp"
#include "./symbol_proc_ids.hpp"
#include "../../include/TypeManager.hpp"
#include "../sema/ScopeManager.hpp"
...
namespace pcit::panther{
	...
	struct SymbolProcInstruction{
		...
		struct FuncDecl{
			...
			[[nodiscard]] auto params() const -> evo::ArrayProxy<std::optional<SymbolProcTypeID>> {
				return evo::ArrayProxy<std::optional<SymbolProcTypeID>>(
					this->types.data(), this->func_def.params.size()
				);
			}

			[[nodiscard]] auto returns() const -> evo::ArrayProxy<SymbolProcTypeID> {
			...
			[[nodiscard]] auto error_returns() const -> evo::ArrayProxy<SymbolProcTypeID> {
		...
		struct ForUnrollCond{
			...
			[[nodiscard]] auto get_index() const -> size_t { return this->index; }
	...
	class SymbolProc{
		...
		public:
			...
			[[nodiscard]] auto getID() const -> ID { return this->this_id; }
			[[nodiscard]] auto getASTNode() const -> AST::Node { return this->ast_node; }
			[[nodiscard]] auto getSourceID() const -> SourceID { return this->source_id; }
			[[nodiscard]] auto getIdent() const -> std::string_view { return this->ident; }
			[[nodiscard]] auto isLocalSymbol() const -> bool { return this->is_local_symbol; }

			[[nodiscard]] auto isPriority() const -> bool { return this->is_always_priority; }


			[[nodiscard]] auto getInstruction() const -> const Instruction& {
			...
			[[nodiscard]] auto isTemplateSubSymbol() const -> bool {
			...
			[[nodiscard]] auto isPIRDeclDone() const -> bool {
			...
			[[nodiscard]] auto isWaiting() const -> bool {
				const auto lock = std::scoped_lock(this->waiting_for_lock);
				return this->waiting_for.empty() == false;
			}

			[[nodiscard]] auto isReadyToBeAddedToWorkQueue() const -> bool {
			...
			[[nodiscard]] auto shouldContinueRunning() -> bool {
			...
			enum class BuiltinSymbolKind{
				...
				ARRAY_REF_ITERABLE_REF_CT,
				ARRAY_MUT_REF_ITERABLE_MUT_REF,
				ARRAY_MUT_REF_ITERABLE_MUT_REF_RT,
				ARRAY_MUT_REF_ITERABLE_MUT_REF_CT,

				_LAST_ = ARRAY_MUT_REF_ITERABLE_MUT_REF_CT,
			};
