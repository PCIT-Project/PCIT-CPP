...
namespace pcit::panther{
	...
	struct ManagedLifetimeErrorParam{
		...
		[[nodiscard]] auto operator==(const ManagedLifetimeErrorParam&) const -> bool = default;
	};

	struct OpDeleteThisAccessor{
		uint32_t abiIndex;

		[[nodiscard]] auto operator==(const OpDeleteThisAccessor&) const -> bool = default;
	};
...
namespace pcit::panther{
	...
	class SemaToPIR{
		...
		private:
			...
			[[nodiscard]] auto get_expr_register(const sema::Expr expr) -> pir::Expr;
			[[nodiscard]] auto get_expr_pointer(const sema::Expr expr) -> pir::Expr;
			[[nodiscard]] auto get_expr_store(const sema::Expr expr, evo::ArrayProxy<pir::Expr> store_locations)
				-> void;
			[[nodiscard]] auto get_expr_discard(const sema::Expr expr) -> void;
			...
			[[nodiscard]] auto calc_in_param_bitmap(
			...
			[[nodiscard]] auto get_type(TypeInfo::VoidableID voidable_type_id) -> PIRType;

			template<bool MAY_LOWER_DEPENDENCY, bool GET_META>
			[[nodiscard]] auto get_type(TypeInfo::ID type_id) -> PIRType;

			template<bool MAY_LOWER_DEPENDENCY, bool GET_META>
			[[nodiscard]] auto get_type(BaseType::ID base_type_id) -> PIRType;
			...
			[[nodiscard]] auto get_location(const Diagnostic::Location& location) const -> Location;

			[[nodiscard]] auto get_current_meta_scope() const -> pir::meta::Scope;
			[[nodiscard]] auto get_current_meta_local_scope() const -> pir::meta::LocalScope;
			...
			template<bool PIR_STMT_NAME_SAFE = false>
			[[nodiscard]] auto mangle_name(sema::GlobalVar::ID global_var_id) const -> std::string;

			template<bool PIR_STMT_NAME_SAFE = false>
			[[nodiscard]] auto mangle_name(sema::Func::ID func_id) const -> std::string;

			template<bool PIR_STMT_NAME_SAFE = false>
			[[nodiscard]] auto mangle_name(sema::Func::ID func_id, Token::Kind op_kind) const -> std::string;
			...
			[[nodiscard]] auto name(std::string_view str) const -> std::string;
			template<class... Args>
			[[nodiscard]] auto name(std::format_string<Args...> fmt, Args&&... args) const -> std::string;
			...
			[[nodiscard]] auto get_current_scope_level() -> ScopeLevel&;
