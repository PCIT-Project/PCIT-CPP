	...
	class API{
		public:
			...
			struct Decl{
				...
				template<class T>
				[[nodiscard]] auto is() const -> bool { return this->ptr.is<T*>(); }

				template<class T>
				[[nodiscard]] auto as() const -> const T& { return *this->ptr.as<T*>(); }

				template<class T>
				[[nodiscard]] auto as() -> T& { return *this->ptr.as<T*>(); }
		...
		public:
			...
			[[nodiscard]] auto getDecls() const -> evo::ArrayProxy<Decl> { return this->decls; }

			[[nodiscard]] auto getMacros() const -> evo::IterRange<evo::StepVector<Macro>::const_iterator> {
				return evo::IterRange<evo::StepVector<Macro>::const_iterator>(this->macros.begin(), this->macros.end());
			}



			[[nodiscard]] auto lookupSpecialPrimitive(std::string_view name) const 
