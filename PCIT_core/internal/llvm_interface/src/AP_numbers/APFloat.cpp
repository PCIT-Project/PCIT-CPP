...
#define APFLOAT_DISPATCH_ON_SEMANTICS(METHOD_CALL)                             \
	do {                                                                         \
		...
		evo::debugFatalBreak("Unexpected semantics");                                  \
	} while (false)
...
namespace pcit::llvmint{
	...
	/* These are legacy semantics for the fallback, inaccrurate implementation of
		 ...
		 semantics.  */
	static constexpr fltSemantics semPPCDoubleDoubleLegacy = {1023, -1022 + 53, 53 + 53, 128};

	const fltSemantics &APFloatBase::EnumToSemantics(Semantics S) {
		switch (S) {
		case S_IEEEhalf:
			return IEEEhalf();
		case S_BFloat:
		...
		evo::debugFatalBreak("Unrecognised floating semantics");
	}

	APFloatBase::Semantics
	APFloatBase::SemanticsToEnum(const fltSemantics &Sem) {
		if (&Sem == &APFloat::IEEEhalf())
			return S_IEEEhalf;
		else if (&Sem == &APFloat::BFloat())
			return S_BFloat;
		else if (&Sem == &APFloat::IEEEsingle())
			return S_IEEEsingle;
		else if (&Sem == &APFloat::IEEEdouble())
			return S_IEEEdouble;
		else if (&Sem == &APFloat::IEEEquad())
			return S_IEEEquad;
		else if (&Sem == &APFloat::PPCDoubleDouble())
			return S_PPCDoubleDouble;
		else if (&Sem == &APFloat::Float8E5M2())
			return S_Float8E5M2;
		else if (&Sem == &APFloat::Float8E5M2FNUZ())
			return S_Float8E5M2FNUZ;
		else if (&Sem == &APFloat::Float8E4M3FN())
			return S_Float8E4M3FN;
		else if (&Sem == &APFloat::Float8E4M3FNUZ())
			return S_Float8E4M3FNUZ;
		else if (&Sem == &APFloat::Float8E4M3B11FNUZ())
			return S_Float8E4M3B11FNUZ;
		else if (&Sem == &APFloat::FloatTF32())
			return S_FloatTF32;
		else if (&Sem == &APFloat::x87DoubleExtended())
			return S_x87DoubleExtended;
		else
			evo::debugFatalBreak("Unknown floating semantics");
	}
	...
	constexpr glue::RoundingMode APFloatBase::rmNearestTiesToEven;
	constexpr glue::RoundingMode APFloatBase::rmTowardPositive;
	constexpr glue::RoundingMode APFloatBase::rmTowardNegative;
	constexpr glue::RoundingMode APFloatBase::rmTowardZero;
	constexpr glue::RoundingMode APFloatBase::rmNearestTiesToAway;
	...
	unsigned int APFloatBase::semanticsIntSizeInBits(const fltSemantics &semantics, bool isSigned) {
	...
	bool APFloatBase::isRepresentableAsNormalIn(const fltSemantics &Src, const fltSemantics &Dst) {
...
namespace pcit::llvmint{
	...
	*/
	...
	{
		evo::debugAssert(HUerr1 < 2 || HUerr2 < 2 || (HUerr1 + HUerr2 < 8));
	...
	powerOf5(APFloatBase::integerPart *dst, unsigned int power) {
		...
		unsigned int result;
		evo::debugAssert(power <= maxExponent);
						 ...
						 const char *hexDigitChars)
	{
		...
		evo::debugAssert(count != 0 && count <= APFloatBase::integerPartWidth / 4);
	...
	void IEEEFloat::assign(const IEEEFloat &rhs) {
		evo::debugAssert(semantics == rhs.semantics);
	...
	void IEEEFloat::copySignificand(const IEEEFloat &rhs) {
		evo::debugAssert(isFiniteNonZero() || category == fcNaN);
		evo::debugAssert(rhs.partCount() >= partCount());
	...
	bool IEEEFloat::isSignificandAllOnes() const {
		...
		evo::debugAssert(NumHighBits <= integerPartWidth && NumHighBits > 0 &&
	...
	bool IEEEFloat::isSignificandAllZeros() const {
		...
		const unsigned NumHighBits =
			PartCount*integerPartWidth - semantics->precision + 1;
		evo::debugAssert(NumHighBits < integerPartWidth, "Can not have more high bits to "
	...
	IEEEFloat::integerPart IEEEFloat::addSignificand(const IEEEFloat &rhs) {
		...
		evo::debugAssert(semantics == rhs.semantics);
		evo::debugAssert(exponent == rhs.exponent);
	...
	/* Subtract the significand of the RHS with a borrow flag.  Returns
		 the borrow flag.  */
	IEEEFloat::integerPart IEEEFloat::subtractSignificand(const IEEEFloat &rhs, integerPart borrow) {
		integerPart *parts;
	...
	/* Multiply the significand of the RHS.  If ADDEND is non-NULL, add it
		 ...
		 lost fraction.  */
	lostFraction IEEEFloat::multiplySignificand(const IEEEFloat &rhs, IEEEFloat addend) {
		...
		if (addend.isNonZero()) {
			...
			if (omsb != extendedPrecision - 1) {
				evo::debugAssert(extendedPrecision > omsb);
			...
			evo::debugAssert(lost_fraction == lfExactlyZero &&
	...
	lostFraction IEEEFloat::divideSignificand(const IEEEFloat &rhs) {
		...
		if (APInt::tcCompare(dividend, divisor, partsCount) < 0) {
			...
			evo::debugAssert(APInt::tcCompare(dividend, divisor, partsCount) >= 0);
		}
	...
	lostFraction IEEEFloat::shiftSignificandRight(unsigned int bits) {
		...
		evo::debugAssert((ExponentType) (exponent + bits) >= exponent);
	...
	void IEEEFloat::shiftSignificandLeft(unsigned int bits) {
		evo::debugAssert(bits < semantics->precision);
		...
		if (bits) {
			...
			evo::debugAssert(!APInt::tcIsZero(significandParts(), partsCount));
		}
	}
	...
	IEEEFloat::compareAbsoluteValue(const IEEEFloat &rhs) const {
		...
		evo::debugAssert(semantics == rhs.semantics);
		evo::debugAssert(isFiniteNonZero());
		evo::debugAssert(rhs.isFiniteNonZero());
	...
	bool IEEEFloat::roundAwayFromZero(roundingMode rounding_mode,
		...
		evo::debugAssert(isFiniteNonZero() || category == fcZero);
		...
		evo::debugFatalBreak("Invalid rounding mode found");
	}
	...
	IEEEFloat::opStatus IEEEFloat::normalize(roundingMode rounding_mode, lostFraction lost_fraction) {
		...
		evo::debugAssert(omsb < semantics->precision);
	...
	lostFraction IEEEFloat::addOrSubtractSignificand(const IEEEFloat &rhs, bool subtract) {
		integerPart carry;
		lostFraction lost_fraction;
		int bits;
	...
	IEEEFloat::opStatus IEEEFloat::addOrSubtract(const IEEEFloat &rhs, roundingMode rounding_mode, bool subtract) {
		opStatus fs;
		...
		if (fs == opDivByZero) {
			...
			evo::debugAssert(category != fcZero || lost_fraction == lfExactlyZero);
		}
	...
	/* Normalized addition.  */
	IEEEFloat::opStatus IEEEFloat::add(const IEEEFloat &rhs, roundingMode rounding_mode) {
		return addOrSubtract(rhs, rounding_mode, false);
	}

	/* Normalized subtraction.  */
	IEEEFloat::opStatus IEEEFloat::subtract(const IEEEFloat &rhs, roundingMode rounding_mode) {
		return addOrSubtract(rhs, rounding_mode, true);
	}

	/* Normalized multiply.  */
	IEEEFloat::opStatus IEEEFloat::multiply(const IEEEFloat &rhs, roundingMode rounding_mode) {
		opStatus fs;
	...
	/* Normalized divide.  */
	IEEEFloat::opStatus IEEEFloat::divide(const IEEEFloat &rhs, roundingMode rounding_mode) {
		opStatus fs;
	...
	IEEEFloat::opStatus IEEEFloat::remainder(const IEEEFloat &rhs) {
		...
		evo::debugAssert(fs == opOK && !losesInfo);
		IEEEFloat PEx = P;
	...
	IEEEFloat::opStatus IEEEFloat::fusedMultiplyAdd(
		const IEEEFloat &multiplicand, const IEEEFloat &addend, roundingMode rounding_mode
	) {
		opStatus fs;
	...
	IEEEFloat::opStatus IEEEFloat::convert(const fltSemantics &toSemantics,
		...
		if(
			shift < 0 && 
			(isFiniteNonZero() || (category == fcNaN && semantics->nonFiniteBehavior != fltNonfiniteBehavior::NanOnly))
		){
	...
	IEEEFloat::opStatus IEEEFloat::convertToSignExtendedInteger(
			std::span<integerPart> parts, unsigned int width, bool isSigned,
		...
		*isExact = false;
		...
		evo::debugAssert(dstPartsCount <= parts.size(), "Integer too big");
	...
	*/
	IEEEFloat::opStatus
	IEEEFloat::convertToInteger(std::span<integerPart> parts,
	...
	IEEEFloat::convertFromZeroExtendedInteger(const integerPart *parts,
		...
		APInt api = APInt(width, evo::ArrayProxy(parts, partCount));
	...
	IEEEFloat::roundSignificandWithExponent(const integerPart *decSigParts,
		...
		for (;; parts *= 2) {
			...
			evo::debugAssert(APInt::tcExtractBit
	...
	// bool IEEEFloat::convertFromStringSpecials(std::string_view str) {
	...
	*/
	...
	char *IEEEFloat::convertNormalToHexString(
		char *dst, unsigned int hexDigits, bool upperCase, roundingMode rounding_mode
	) const {
	...
	APInt IEEEFloat::convertF80LongDoubleAPFloatToAPInt() const {
		evo::debugAssert(semantics == (const fltSemantics*)&semX87DoubleExtended);
		evo::debugAssert(partCount()==2);
		...
		if (isFiniteNonZero()) {
			...
			evo::debugAssert(category == fcNaN, "Unknown category");
			myexponent = 0x7fff;
			mysignificand = significandParts()[0];
		}

		// uint64_t words[2];
		auto words = std::array<uint64_t, 2>();
	...
	APInt IEEEFloat::convertPPCDoubleDoubleAPFloatToAPInt() const {
		evo::debugAssert(semantics == (const fltSemantics *)&semPPCDoubleDoubleLegacy);
		evo::debugAssert(partCount()==2);
		...
		evo::debugAssert(fs == opOK || fs == opInexact);
		(void)fs;
	...
	APInt IEEEFloat::convertIEEEFloatToAPInt() const {
		...
		if (isFiniteNonZero()) {
			...
			if (S.nonFiniteBehavior == fltNonfiniteBehavior::NanOnly) {
				evo::debugFatalBreak("semantics don't support inf!");
			}
	...
	APInt IEEEFloat::bitcastToAPInt() const {
		if (semantics == (const fltSemantics*)&semIEEEhalf)
			return convertHalfAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semBFloat)
			return convertBFloatAPFloatToAPInt();

		if (semantics == (const fltSemantics*)&semIEEEsingle)
			return convertFloatAPFloatToAPInt();

		if (semantics == (const fltSemantics*)&semIEEEdouble)
			return convertDoubleAPFloatToAPInt();

		if (semantics == (const fltSemantics*)&semIEEEquad)
			return convertQuadrupleAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semPPCDoubleDoubleLegacy)
			return convertPPCDoubleDoubleAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semFloat8E5M2)
			return convertFloat8E5M2APFloatToAPInt();

		if (semantics == (const fltSemantics *)&semFloat8E5M2FNUZ)
			return convertFloat8E5M2FNUZAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semFloat8E4M3FN)
			return convertFloat8E4M3FNAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semFloat8E4M3FNUZ)
			return convertFloat8E4M3FNUZAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semFloat8E4M3B11FNUZ)
			return convertFloat8E4M3B11FNUZAPFloatToAPInt();

		if (semantics == (const fltSemantics *)&semFloatTF32)
	...
	float IEEEFloat::convertToFloat() const {
		evo::debugAssert(semantics == (const fltSemantics*)&semIEEEsingle &&
	...
	double IEEEFloat::convertToDouble() const {
		evo::debugAssert(semantics == (const fltSemantics*)&semIEEEdouble &&
	...
	void IEEEFloat::initFromIEEEAPInt(const APInt &api) {
		evo::debugAssert(api.getBitWidth() == S.sizeInBits);
		...
		initialize(&S);
		evo::debugAssert(partCount() == mysignificand.size());
	...
	void IEEEFloat::initFromAPInt(const fltSemantics *Sem, const APInt &api) {
		evo::debugAssert(api.getBitWidth() == Sem->sizeInBits);
		if (Sem == &semIEEEhalf)
	...
	namespace {
		void append(std::string &Buffer, StringRef Str) {
		...
		void AdjustToPrecision(std::string &buffer, int &exp, unsigned FormatPrecision) {
			...
			if (buffer[FirstSignificant - 1] < '5') {
				...
				buffer.erase(buffer.begin(), buffer.begin() + FirstSignificant);
				return;
			}
	...
	void IEEEFloat::toString(
		std::string &Str, unsigned FormatPrecision, unsigned FormatMaxPadding, bool TruncateZero
	) const {
		switch (category) {
		case fcInfinity:
			if (isNegative())
		...
		APInt significand(
				semantics->precision,
				evo::ArrayProxy(significandParts(), partCountForBits(semantics->precision)));
		...
		evo::debugAssert(!buffer.empty(), "no characters in buffer!");
	...
	bool IEEEFloat::getExactInverse(APFloat *inv) const {
		...
		evo::debugAssert(reciprocal.isFiniteNonZero() &&
	...
	int IEEEFloat::getExactLog2Abs() const {
		...
		evo::debugFatalBreak("didn't find the set bit");
	}
		...
		case fcNormal:
			...
			if (isNegative()) {
				...
				if (WillCrossBinadeBoundary) {
					...
					APInt::tcSetBit(Parts, semantics->precision - 1);
					evo::debugAssert(exponent != semantics->maxExponent &&
	...
	DoubleAPFloat::DoubleAPFloat(const fltSemantics &S)
		...
		evo::debugAssert(Semantics == &semPPCDoubleDouble);
	}
	...
	DoubleAPFloat::DoubleAPFloat(const fltSemantics &S, APFloat &&First,
		...
		evo::debugAssert(Semantics == &semPPCDoubleDouble);
		evo::debugAssert(&Floats[0].getSemantics() == &semIEEEdouble);
		evo::debugAssert(&Floats[1].getSemantics() == &semIEEEdouble);
	}
	...
	APFloat::opStatus DoubleAPFloat::addWithSpecial(const DoubleAPFloat &LHS,
		...
		evo::debugAssert(LHS.getCategory() == fcNormal && RHS.getCategory() == fcNormal);

		APFloat A(LHS.Floats[0]), AA(LHS.Floats[1]), C(RHS.Floats[0]),
				CC(RHS.Floats[1]);
		evo::debugAssert(&A.getSemantics() == &semIEEEdouble);
		evo::debugAssert(&AA.getSemantics() == &semIEEEdouble);
		evo::debugAssert(&C.getSemantics() == &semIEEEdouble);
		evo::debugAssert(&CC.getSemantics() == &semIEEEdouble);
		evo::debugAssert(&Out.Floats[0].getSemantics() == &semIEEEdouble);
		evo::debugAssert(&Out.Floats[1].getSemantics() == &semIEEEdouble);
	...
	APFloat::opStatus DoubleAPFloat::divide(const DoubleAPFloat &RHS, APFloat::roundingMode RM) {
		evo::debugAssert(Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
	...
	// Expected<APFloat::opStatus> DoubleAPFloat::convertFromString(StringRef S, roundingMode RM) {
	// 	evo::debugAssert(Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
	...
	APFloat::opStatus
	DoubleAPFloat::convertToInteger(std::span<integerPart> Input,
																	unsigned int Width, bool IsSigned,
	...
	APFloat::opStatus DoubleAPFloat::convertFromAPInt(const APInt &Input, bool IsSigned, roundingMode RM) {
		evo::debugAssert(Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
		APFloat Tmp(semPPCDoubleDoubleLegacy);
	...
	APFloat::opStatus
	DoubleAPFloat::convertFromSignExtendedInteger(
		const integerPart *Input, unsigned int InputSize, bool IsSigned, roundingMode RM
	) {
		evo::debugAssert(Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
		APFloat Tmp(semPPCDoubleDoubleLegacy);
	...
	APFloat::opStatus
	DoubleAPFloat::convertFromZeroExtendedInteger(
		const integerPart *Input, unsigned int InputSize, bool IsSigned, roundingMode RM
	) {
		evo::debugAssert(Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
		APFloat Tmp(semPPCDoubleDoubleLegacy);
	...
	unsigned int DoubleAPFloat::convertToHexString(
		char *DST, unsigned int HexDigits, bool UpperCase, roundingMode RM
	) const {
		evo::debugAssert(Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
		return APFloat(semPPCDoubleDoubleLegacy, bitcastToAPInt())
	...
	void DoubleAPFloat::toString(std::string &Str,
	...
	DoubleAPFloat scalbn(const DoubleAPFloat &Arg, int Exp,
											 APFloat::roundingMode RM) {
		evo::debugAssert(Arg.Semantics == &semPPCDoubleDouble, "Unexpected Semantics");
	...
	// Expected<APFloat::opStatus> APFloat::convertFromString(StringRef Str, roundingMode RM) {
	...
	// APFloat::APFloat(const fltSemantics &Semantics, std::string_view S) : APFloat(Semantics) {
	// 	auto StatusOrErr = convertFromString(S, rmNearestTiesToEven);
	// 	evo::debugAssert(StatusOrErr, "Invalid floating point representation");
	...
	// 	evo::debugAssert(isNaN(), "Other class of FP constant");
	// 	return isSignaling() ? fcSNan : fcQNan;
	// }

	APFloat::opStatus APFloat::convert(const fltSemantics &ToSemantics, roundingMode RM, bool *losesInfo) {
		...
		if (usesLayout<IEEEFloat>(getSemantics()) &&
			...
			evo::debugAssert(&ToSemantics == &semPPCDoubleDouble);
	 ...
	 */
	// APFloat::opStatus APFloat::convertToInteger(APSInt &result, roundingMode rounding_mode, bool *isExact) const {
	...
	double APFloat::convertToDouble() const {
		if (&getSemantics() == (const fltSemantics *)&semIEEEdouble)
			return getIEEE().convertToDouble();
		evo::debugAssert(getSemantics().isRepresentableBy(semIEEEdouble) &&
	...
	float APFloat::convertToFloat() const {
		if (&getSemantics() == (const fltSemantics *)&semIEEEsingle)
			return getIEEE().convertToFloat();
		evo::debugAssert(getSemantics().isRepresentableBy(semIEEEsingle) &&
