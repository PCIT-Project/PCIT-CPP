...
namespace pcit::llvmint{
	...
	auto APInt::fromNative(const llvm::APInt& native) -> APInt {
		// TODO: better way to do this?
		return (const APInt&)native;
	}
	...
	void APInt::initFromArray(evo::ArrayProxy<uint64_t> bigVal) {
		evo::debugAssert(bigVal.data(), "Null pointer detected!");
		if (isSingleWord())
			U.VAL = bigVal[0];
		else {
	...
	APInt::APInt(unsigned numBits, evo::ArrayProxy<uint64_t> bigVal) : BitWidth(numBits) {
		initFromArray(bigVal);
	}

	APInt::APInt(unsigned numBits, unsigned numWords, const uint64_t bigVal[])
			: BitWidth(numBits) {
		initFromArray(evo::ArrayProxy(bigVal, numWords));
	}

	APInt::APInt(unsigned numbits, std::string_view Str, uint8_t radix)
			: BitWidth(numbits) {
	...
	APInt& APInt::operator+=(const APInt& RHS) {
		evo::debugAssert(BitWidth == RHS.BitWidth, "Bit widths must be the same");
		if (isSingleWord())
			U.VAL += RHS.U.VAL;
		else
	...
	int APInt::compare(const APInt& RHS) const {
		evo::debugAssert(BitWidth == RHS.BitWidth, "Bit widths must be same for comparison");
		if (isSingleWord())
	...
	int APInt::compareSigned(const APInt& RHS) const {
		evo::debugAssert(BitWidth == RHS.BitWidth, "Bit widths must be same for comparison");
		if (isSingleWord()) {
			int64_t lhsSext = glue::SignExtend64(U.VAL, BitWidth);
			int64_t rhsSext = glue::SignExtend64(RHS.U.VAL, BitWidth);
	...
	void APInt::flipBit(unsigned bitPosition) {
		evo::debugAssert(bitPosition < BitWidth, "Out of the bit-width range!");
	...
	void APInt::insertBits(const APInt &subBits, unsigned bitPosition) {
		...
		evo::debugAssert((subBitWidth + bitPosition) <= BitWidth, "Illegal bit insertion");
	...
	void APInt::insertBits(uint64_t subBits, unsigned bitPosition, unsigned numBits) {
		uint64_t maskBits = llvm::maskTrailingOnes<uint64_t>(numBits);
		subBits &= maskBits;
		if (isSingleWord()) {
	...
	APInt APInt::extractBits(unsigned numBits, unsigned bitPosition) const {
		evo::debugAssert(bitPosition < BitWidth && (numBits + bitPosition) <= BitWidth &&
		...
		if (loBit == 0)
			return APInt(numBits, evo::ArrayProxy/*<const uint64_t*>*/(U.pVal + loWord, size_t(1 + hiWord - loWord)));
	...
	uint64_t APInt::extractBitsAsZExtValue(unsigned numBits,
		...
		evo::debugAssert(bitPosition < BitWidth && (numBits + bitPosition) <= BitWidth &&
					 "Illegal bit extraction");
		evo::debugAssert(numBits <= 64, "Illegal bit extraction");
	...
	unsigned APInt::getSufficientBitsNeeded(std::string_view Str, uint8_t Radix) {
		evo::debugAssert(!Str.empty(), "Invalid string length");
		...
		if (Str[0] == '-' || Str[0] == '+') {
			IsNegative = Str[0] == '-';
			StrLen--;
			evo::debugAssert(StrLen, "String is only a sign, needs a value.");
		}
		...
		evo::debugAssert(Radix == 36);
		return (StrLen == 1 ? 7 : StrLen * 16 / 3) + IsNegative;
	}

	unsigned APInt::getBitsNeeded(std::string_view str, uint8_t radix) {
		// Compute a sufficient number of bits that is always large enough but might
		// be too large.
		...
		if (*p == '-' || *p == '+') {
			p++;
			slen--;
			evo::debugAssert(slen, "String is only a sign, needs a value.");
		}


		// Convert to the actual binary value.
		APInt tmp(sufficient, std::string_view(&*p, slen), radix);
	...
	bool APInt::isSplat(unsigned SplatSizeInBits) const {
		evo::debugAssert(getBitWidth() % SplatSizeInBits == 0 &&
	...
	APInt APInt::getSplat(unsigned NewLen, const APInt &V) {
		evo::debugAssert(NewLen >= V.getBitWidth(), "Can't splat to smaller bit width!");
	...
	unsigned APInt::countLeadingOnesSlowCase() const {
		...
		int i = getNumWords() - 1;
		unsigned Count = std::countl_one(U.pVal[i] << shift);
		if (Count == highWordBits) {
			for (i--; i >= 0; --i) {
				if (U.pVal[i] == WORDTYPE_MAX)
					Count += APINT_BITS_PER_WORD;
				else {
					Count += std::countl_one(U.pVal[i]);
					break;
				}
			}
		}
	...
	unsigned APInt::countTrailingZerosSlowCase() const {
		...
		for (; i < getNumWords() && U.pVal[i] == 0; ++i)
			Count += APINT_BITS_PER_WORD;
		if (i < getNumWords())
			Count += std::countr_zero(U.pVal[i]);
	...
	unsigned APInt::countTrailingOnesSlowCase() const {
		...
		for (; i < getNumWords() && U.pVal[i] == WORDTYPE_MAX; ++i)
			Count += APINT_BITS_PER_WORD;
		if (i < getNumWords())
			Count += std::countr_one(U.pVal[i]);
		evo::debugAssert(Count <= BitWidth);
		return Count;
	}
	...
	APInt APInt::byteSwap() const {
		evo::debugAssert(BitWidth >= 16 && BitWidth % 8 == 0, "Cannot byteswap!");
		if (BitWidth == 16)
	...
	APInt APIntOps::GreatestCommonDivisor(APInt A, APInt B) {
	...
	APInt APIntOps::RoundDoubleToAPInt(double Double, unsigned width) {
		uint64_t I = std::bit_cast<uint64_t>(Double);
	...
	double APInt::roundToDouble(bool isSigned) const {
		...
		if (isSingleWord() || getActiveBits() <= APINT_BITS_PER_WORD) {
			if (isSigned) {
				int64_t sext = glue::SignExtend64(getWord(0), BitWidth);
				return double(sext);
			} else
	...
	APInt APInt::trunc(unsigned width) const {
		evo::debugAssert(width <= BitWidth, "Invalid APInt Truncate request");
	...
	APInt APInt::sext(unsigned Width) const {
		evo::debugAssert(Width >= BitWidth, "Invalid APInt SignExtend request");

		if (Width <= APINT_BITS_PER_WORD)
			return APInt(Width, glue::SignExtend64(U.VAL, BitWidth));
		...
		Result.U.pVal[getNumWords() - 1] =
				glue::SignExtend64(Result.U.pVal[getNumWords() - 1],
	...
	APInt APInt::zext(unsigned width) const {
		evo::debugAssert(width >= BitWidth, "Invalid APInt ZeroExtend request");
	...
	void APInt::ashrSlowCase(unsigned ShiftAmt) {
		...
		if (WordsToMove != 0) {
			...
			U.pVal[getNumWords() - 1] = glue::SignExtend64(
			...
			if (BitShift == 0) {
				...
				U.pVal[WordsToMove - 1] =
						glue::SignExtend64(U.pVal[WordsToMove - 1], APINT_BITS_PER_WORD - BitShift);
			}
		}
	...
	APInt APInt::sqrt() const {
		...
		if (magnitude < 52) {
			return APInt(BitWidth, uint64_t(::round(::sqrt(double(isSingleWord() ? U.VAL : U.pVal[0])))));
		}
		...
		if (this->ult(square))
			return x_old;
		evo::debugAssert(this->ule(nextSquare), "Error in APInt::sqrt computation");
	...
	APInt APInt::multiplicativeInverse(const APInt& modulo) const {
		evo::debugAssert(ult(modulo), "This APInt must be smaller than the modulo");
	...
	static void KnuthDiv(uint32_t *u, uint32_t *v, uint32_t *q, uint32_t* r,
		...
		evo::debugAssert(u, "Must provide dividend");
		evo::debugAssert(v, "Must provide divisor");
		evo::debugAssert(q, "Must provide quotient");
		evo::debugAssert(u != v && u != q && v != q, "Must use different memory");
		evo::debugAssert(n>1, "n must be > 1");
	...
	void APInt::divide(const WordType *LHS, unsigned lhsWords, const WordType *RHS,
		...
		evo::debugAssert(lhsWords >= rhsWords, "Fractional result");
		...
		evo::debugAssert(n != 0, "Divide by zero?");
		if (n == 1) {
	...
	APInt APInt::udiv(const APInt &RHS) const {
		...
		if (isSingleWord()) {
			evo::debugAssert(RHS.U.VAL != 0, "Divide by zero?");
			return APInt(BitWidth, U.VAL / RHS.U.VAL);
		}
		...
		evo::debugAssert(rhsWords, "Divided by zero???");
	...
	APInt APInt::udiv(uint64_t RHS) const {
		evo::debugAssert(RHS != 0, "Divide by zero?");
	...
	APInt APInt::urem(const APInt &RHS) const {
		evo::debugAssert(BitWidth == RHS.BitWidth, "Bit widths must be the same");
		if (isSingleWord()) {
			evo::debugAssert(RHS.U.VAL != 0, "Remainder by zero?");
			return APInt(BitWidth, U.VAL % RHS.U.VAL);
		}
		...
		unsigned rhsBits = RHS.getActiveBits();
		unsigned rhsWords = getNumWords(rhsBits);
		evo::debugAssert(rhsWords, "Performing remainder operation by zero ???");
	...
	uint64_t APInt::urem(uint64_t RHS) const {
		evo::debugAssert(RHS != 0, "Remainder by zero?");
	...
	void APInt::udivrem(const APInt &LHS, const APInt &RHS,
		...
		evo::debugAssert(LHS.BitWidth == RHS.BitWidth, "Bit widths must be the same");
		...
		unsigned rhsBits  = RHS.getActiveBits();
		unsigned rhsWords = getNumWords(rhsBits);
		evo::debugAssert(rhsWords, "Performing divrem operation by zero ???");
	...
	void APInt::fromString(unsigned numbits, std::string_view str, uint8_t radix) {
		// Check our assumptions here
		evo::debugAssert(!str.empty(), "Invalid string length");
		evo::debugAssert((radix == 10 || radix == 8 || radix == 16 || radix == 2 ||
						radix == 36) &&
		...
		evo::debugAssert((slen <= numbits || radix != 2), "Insufficient bit width");
		evo::debugAssert(((slen-1)*3 <= numbits || radix != 8), "Insufficient bit width");
		evo::debugAssert(((slen-1)*4 <= numbits || radix != 16), "Insufficient bit width");
		evo::debugAssert((((slen-1)*64)/22 <= numbits || radix != 10) &&
		...
		for (std::string_view::iterator e = str.end(); p != e; ++p) {
			...
			evo::debugAssert(digit < radix, "Invalid character in digit string");
	...
	void APInt::toString(std::string &Str, unsigned Radix, bool Signed,
		...
		if (formatAsCLiteral) {
			switch (Radix) {
				...
				case 16:
					...
					break;
				default:
					evo::debugFatalBreak("Invalid radix!");
			}
		}
		...
		if (Radix == 2 || Radix == 8 || Radix == 16) {
			...
			while (Tmp.getBoolValue()) {
				...
				evo::debugAssert(Digit < Radix, "divide failed");
				if (InsertSeparators && Pos % Grouping == 0 && Pos > 0)
					Str.push_back('\'');
	...
	static inline APInt::WordType lowBitMask(unsigned bits) {
		evo::debugAssert(bits != 0 && bits <= APInt::APINT_BITS_PER_WORD);
	...
	APInt::tcExtract(WordType *dst, unsigned dstCount, const WordType *src,
		...
		evo::debugAssert(dstParts <= dstCount);
	...
	int APInt::tcMultiplyPart(WordType *dst, const WordType *src,
		...
		evo::debugAssert(dst <= src || dst >= src + srcParts);
		evo::debugAssert(dstParts <= srcParts + 1);
		...
		if (srcParts < dstParts) {
			...
			evo::debugAssert(srcParts + 1 == dstParts);
			dst[srcParts] = carry;
			return 0;
		}
	...
	int APInt::tcMultiply(WordType *dst, const WordType *lhs,
		...
		evo::debugAssert(dst != lhs && dst != rhs);
	...
	int APInt::tcDivide(WordType *lhs, const WordType *rhs,
											...
											unsigned parts) {
		evo::debugAssert(lhs != remainder && lhs != srhs && remainder != srhs);
	...
	APInt APIntOps::RoundingUDiv(const APInt &A, const APInt &B,
																		 APInt::Rounding RM) {
		...
		evo::debugFatalBreak("Unknown APInt::Rounding enum");
	}

	APInt APIntOps::RoundingSDiv(const APInt &A, const APInt &B,
																		 APInt::Rounding RM) {
		switch (RM) {
		case APInt::Rounding::DOWN:
		case APInt::Rounding::UP: {
	...
	std::optional<APInt>
	APIntOps::SolveQuadraticEquationWrap(APInt A, APInt B, APInt C,
																						 unsigned RangeWidth) {
		unsigned CoeffWidth = A.getBitWidth();
		evo::debugAssert(CoeffWidth == B.getBitWidth() && CoeffWidth == C.getBitWidth());
		evo::debugAssert(RangeWidth <= CoeffWidth &&
					 "Value range width should be less than coefficient width");
		evo::debugAssert(RangeWidth > 1, "Value range bit width should be > 1");
		...
		auto RoundUp = [] (const APInt &V, const APInt &A) -> APInt {
			evo::debugAssert(A.isStrictlyPositive());
		...
		APInt D = SqrB - 4*A*C;
		evo::debugAssert(D.isNonNegative(), "Negative discriminant");
		APInt SQ = D.sqrt();
		...
		evo::debugAssert(X.isNonNegative(), "Solution should be non-negative");
		...
		evo::debugAssert((SQ*SQ).sle(D), "SQ = |_sqrt(D)_|, so SQ*SQ <= D");
	...
	APIntOps::GetMostSignificantDifferentBit(const APInt &A, const APInt &B) {
		evo::debugAssert(A.getBitWidth() == B.getBitWidth(), "Must have the same bitwidth");
		if (A == B)
			return std::nullopt;
		return A.getBitWidth() - ((A ^ B).countl_zero() + 1);
	}

	APInt APIntOps::ScaleBitMask(const APInt &A, unsigned NewBitWidth,
																		 bool MatchAllBits) {
		unsigned OldBitWidth = A.getBitWidth();
		evo::debugAssert((((OldBitWidth % NewBitWidth) == 0) ||
	...
	// 														unsigned StoreBytes) {
	// 	evo::debugAssert((IntVal.getBitWidth()+7)/8 >= StoreBytes, "Integer too small!");
	...
	// 														 unsigned LoadBytes) {
	// 	evo::debugAssert((IntVal.getBitWidth()+7)/8 >= LoadBytes, "Integer too small!");
