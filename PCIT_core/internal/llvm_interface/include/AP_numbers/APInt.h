...
namespace pcit::llvmint{
	...
	public:
		...
		APInt(unsigned numBits, evo::ArrayProxy<uint64_t> bigVal);

		/// Equivalent to APInt(numBits, evo::ArrayProxy<uint64_t>(bigVal, numWords)), but
		...
		APInt(unsigned numBits, std::string_view str, uint8_t radix);

		/// Default constructor that creates an APInt with a 1-bit zero value.
		explicit APInt() { U.VAL = 0; }


		static auto fromNative(const llvm::APInt& native) -> APInt;
		...
		bool isMaxSignedValue() const {
			if (isSingleWord()) {
				evo::debugAssert(BitWidth, "zero width values not allowed");
		...
		bool isPowerOf2() const {
			if (isSingleWord()) {
				evo::debugAssert(BitWidth, "zero width values not allowed");
				return glue::isPowerOf2_64(U.VAL);
			}
		...
		bool isMask(unsigned numBits) const {
			evo::debugAssert(numBits != 0, "numBits must be non-zero");
			evo::debugAssert(numBits <= BitWidth, "numBits out of range");
			if (isSingleWord())
		...
		bool isShiftedMask() const {
			if (isSingleWord())
				return glue::isShiftedMask_64(U.VAL);
		...
		bool isShiftedMask(unsigned &MaskIdx, unsigned &MaskLen) const {
			if (isSingleWord())
				return glue::isShiftedMask_64(U.VAL, MaskIdx, MaskLen);
			unsigned Ones = countPopulationSlowCase();
			unsigned LeadZ = countLeadingZerosSlowCase();
		...
		APInt &operator=(APInt &&that) {
			...
			if (this == &that)
				return *this;
	#endif
			evo::debugAssert(this != &that, "Self-move not supported");
			if (!isSingleWord())
				delete[] U.pVal;
		...
		APInt &operator<<=(unsigned ShiftAmt) {
			evo::debugAssert(ShiftAmt <= BitWidth, "Invalid shift amount");
			if (isSingleWord()) {
		...
		void ashrInPlace(unsigned ShiftAmt) {
			evo::debugAssert(ShiftAmt <= BitWidth, "Invalid shift amount");
			if (isSingleWord()) {
				int64_t SExtVAL = glue::SignExtend64(U.VAL, BitWidth);
				if (ShiftAmt == BitWidth)
		...
		bool operator[](unsigned bitPosition) const {
			evo::debugAssert(bitPosition < getBitWidth(), "Bit position out of bounds!");
		...
		bool operator==(const APInt &RHS) const {
			evo::debugAssert(BitWidth == RHS.BitWidth, "Comparison requires equal bit widths");
			if (isSingleWord())
		...
		void setBit(unsigned BitPosition) {
			evo::debugAssert(BitPosition < BitWidth, "BitPosition out of range");
		...
		void setBitsWithWrap(unsigned loBit, unsigned hiBit) {
			evo::debugAssert(hiBit <= BitWidth, "hiBit out of range");
			evo::debugAssert(loBit <= BitWidth, "loBit out of range");
			if (loBit < hiBit) {
				setBits(loBit, hiBit);
				return;
			}
		...
		void setBits(unsigned loBit, unsigned hiBit) {
			evo::debugAssert(hiBit <= BitWidth, "hiBit out of range");
			evo::debugAssert(loBit <= BitWidth, "loBit out of range");
			evo::debugAssert(loBit <= hiBit, "loBit greater than hiBit");
			if (loBit == hiBit)
				return;
		...
		void clearLowBits(unsigned loBits) {
			evo::debugAssert(loBits <= BitWidth, "More bits than bitwidth");
		...
		uint64_t getZExtValue() const {
			if (isSingleWord())
				return U.VAL;
			evo::debugAssert(getActiveBits() <= 64, "Too many bits for uint64_t");
			return U.pVal[0];
		}
		...
		int64_t getSExtValue() const {
			if (isSingleWord())
				return glue::SignExtend64(U.VAL, BitWidth);
			evo::debugAssert(getSignificantBits() <= 64, "Too many bits for int64_t");
		...
		static unsigned getBitsNeeded(std::string_view str, uint8_t radix);
		...
		static unsigned getSufficientBitsNeeded(std::string_view Str, uint8_t Radix);
		...
		unsigned countl_zero() const {
			if (isSingleWord()) {
				...
				return std::countl_zero(U.VAL) - unusedBits;
			}
		...
		unsigned countl_one() const {
			if (isSingleWord()) {
				if (LLVM_UNLIKELY(BitWidth == 0))
					return 0;
				return std::countl_one(U.VAL << (APINT_BITS_PER_WORD - BitWidth));
			}
		...
		unsigned countr_zero() const {
			if (isSingleWord()) {
				unsigned TrailingZeros = std::countr_zero(U.VAL);
		...
		/// digits.
		void toString(std::string& Str, unsigned Radix, bool Signed,
									bool formatAsCLiteral = false, bool UpperCase = true,
									bool InsertSeparators = false) const;

		/// Considers the APInt to be unsigned and converts it into a string in the
		/// radix given. The radix can be 2, 8, 10 16, or 36.
		void toStringUnsigned(std::string& Str, unsigned Radix = 10) const {
			toString(Str, Radix, false, false);
		}

		/// Considers the APInt to be signed and converts it into a string in the
		/// radix given. The radix can be 2, 8, 10, 16, or 36.
		void toStringSigned(std::string& Str, unsigned Radix = 10) const {
		...
		float bitsToFloat() const {
			return std::bit_cast<float>(static_cast<uint32_t>(getWord(0)));
		}
		...
		static APInt doubleToBits(double V) {
			return APInt(sizeof(double) * CHAR_BIT, std::bit_cast<uint64_t>(V));
		}
		...
		static APInt floatToBits(float V) {
			return APInt(sizeof(float) * CHAR_BIT, std::bit_cast<uint32_t>(V));
		}
	...
	private:
		...
		///
		/// std::string_view::getAsInteger is superficially similar but (1) does
		/// not assume that the string is well-formed and (2) grows the
		/// result to hold the input.
		void fromString(unsigned numBits, std::string_view str, uint8_t radix);
		...
		void initFromArray(evo::ArrayProxy<uint64_t> array);
