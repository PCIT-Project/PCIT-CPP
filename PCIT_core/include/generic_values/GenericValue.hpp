...
namespace pcit::core{
	...
	struct GenericValue{
		...
		[[nodiscard]] static auto createPtr(auto* ptr) -> GenericValue {
			return GenericValue(GenericInt::create<size_t>(reinterpret_cast<size_t>(ptr)));
		}

		[[nodiscard]] static auto createPtr(std::nullptr_t) -> GenericValue {
			return GenericValue(GenericInt::create<size_t>(0));
		}



		[[nodiscard]] static auto createUninit(size_t num_bytes) -> GenericValue {
			return GenericValue(num_bytes);
		}

		[[nodiscard]] static auto createZeroinit(size_t num_bytes) -> GenericValue {
			auto output = GenericValue(num_bytes);
			std::memset(output.writableDataRange().data(), 0, num_bytes);
			return output;
		}

		[[nodiscard]] static auto fromData(evo::ArrayProxy<std::byte> data) -> GenericValue {
		...
		[[nodiscard]] auto getF16() const -> core::GenericFloat {
			return core::GenericFloat::createF16(this->getInt(16));
		}

		[[nodiscard]] auto getF32() const -> core::GenericFloat {
			return core::GenericFloat::createF32(this->getInt(32));
		}

		[[nodiscard]] auto getF64() const -> core::GenericFloat {
			return core::GenericFloat::createF64(this->getInt(64));
		}

		[[nodiscard]] auto getF80() const -> core::GenericFloat {
			return core::GenericFloat::createF80(this->getInt(80));
		}

		[[nodiscard]] auto getF128() const -> core::GenericFloat {
			return core::GenericFloat::createF128(this->getInt(128));
		}


		template<class T>
		[[nodiscard]] auto getPtr() const -> T {
		...
		[[nodiscard]] auto dataRange() const -> evo::ArrayProxy<std::byte> {
			if(this->is_small()){
		...
		[[nodiscard]] auto writableDataRange() -> std::span<std::byte> {
			if(this->is_small()){
		...
		[[nodiscard]] auto operator==(const GenericValue& rhs) const -> bool {
