...
namespace pcit::core{
	...
	class Printer{
		...
		public:
			static [[nodiscard]] auto createConsole(bool with_color) -> Printer {
				if(with_color){
			...
			static [[nodiscard]] auto createConsole() -> Printer {
				return createConsole(platformSupportsColor() == DetectResult::YES);
			}


			static [[nodiscard]] auto createString(bool with_color) -> Printer {
				if(with_color){
			...
			static [[nodiscard]] auto createString() -> Printer {
			...
			[[nodiscard]] static auto platformSupportsColor() -> DetectResult;


			[[nodiscard]] auto isPrintingColor() const -> bool {
				return this->mode == Mode::CONSOLE_COLOR || this->mode == Mode::STRING_COLOR;
			}

			[[nodiscard]] auto isPrintingToConsle() const -> bool {
				return this->mode == Mode::CONSOLE_COLOR || this->mode == Mode::CONSOLE;
			}

			[[nodiscard]] auto isPrintingString() const -> bool {
