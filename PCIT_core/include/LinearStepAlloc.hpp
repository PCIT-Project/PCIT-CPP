...
namespace pcit::core{
	...
	template<class T>
	[[nodiscard]] consteval auto get_optimal_start_pow_of_2_for_linear_step_alloc() -> size_t {
	...
	class LinearStepAlloc{
		...
		private:
			[[nodiscard]] auto size_when_not_fully_deallocated() const -> size_t {
				evo::debugAssert(
			...
			[[nodiscard]] auto get_buffer_and_elem_index(const ID& id) const -> BufferAndElemIndex {
