...
namespace pcit::core{
	...
	class Optional{
		...
		public:
			...
			[[nodiscard]] constexpr auto operator==(const Optional& rhs) const -> bool {
			...
			[[nodiscard]] constexpr auto operator==(const T& rhs) const -> bool {
		...
		private:
			[[nodiscard]] constexpr auto data() const& -> const T&  { return evo::bitCast<T>(this->_data); }
			[[nodiscard]] constexpr auto data()      & ->       T&  { return evo::bitCast<T>(this->_data); }
			[[nodiscard]] constexpr auto data()     && ->       T&& { return evo::bitCast<T>(this->_data); }
	...
	template<class T>
	[[nodiscard]] constexpr auto operator==(const T& lhs, const Optional<T>& rhs) -> bool {
		return rhs == lhs;
	}
}
