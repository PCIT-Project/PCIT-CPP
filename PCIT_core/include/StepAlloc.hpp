...
namespace pcit::core{
	...
	template<class T>
	[[nodiscard]] consteval auto get_optimal_start_pow_of_2_for_step_alloc() -> size_t {
	...
	class StepAlloc{
		public:
			...
			[[nodiscard]] auto emplace_back(auto&&... args) -> ID {
			...
			[[nodiscard]] auto size() const -> size_t { return this->linear_step_alloc.size(); }
			[[nodiscard]] auto empty() const -> bool { return this->linear_step_alloc.empty(); }


			[[nodiscard]] auto clear() -> void {
				const auto lock = std::scoped_lock(this->spin_lock);
			...
			class Iter{
			    public:
			        ...
			        [[nodiscard]] auto getID() const -> ID { return this->index; }
			        [[nodiscard]] auto getParent() const -> StepAlloc& {
			...
			class ConstIter{
			    public:
			        ...
			        [[nodiscard]] auto getID() const -> ID { return this->index; }
			        [[nodiscard]] auto getParent() const -> const StepAlloc& {
			        	evo::debugAssert(this->parent != nullptr, "Iterator was default-constructed; no parent set");
			        	return *this->parent;
			        }
			...
			[[nodiscard]] auto getScopedLock() const -> std::scoped_lock<evo::SpinLock> {
