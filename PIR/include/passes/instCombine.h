...
namespace pcit::pir::passes{
	...
	// 1 + 2 --> 3
	EVO_NODISCARD auto constant_folding_impl(
		class Expr& stmt, class BasicBlock& basic_block, class Function& func, class Module& module
	) -> bool;
	...
	// x + 0 --> x
	// x - x --> 0
	EVO_NODISCARD auto inst_simplify_impl(
		class Expr& stmt, class BasicBlock& basic_block, class Function& func, class Module& module
	) -> bool;
	...
	// x * 4 --> x << 2
	EVO_NODISCARD auto inst_combine_impl(
		class Expr& stmt, class BasicBlock& basic_block, class Function& func, class Module& module
	) -> bool;
